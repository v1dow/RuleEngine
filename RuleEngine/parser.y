...
% union {
	...
	% type<value> expr % type<listvalue> listexpr % type<value> logexpr % type<value> stmt % type<strvalue> strexpr

	% left AND,
	OR % left '+', '-', MORE, LESS, MEQL, LEQL, EQL, NOTEQL, BING, LIAN % left '*', '/' % right UMINUS, NOT

																												% token<value> NUMBER % token<listvalue> LISTNUMBER % token<sym> ID % token<sym> STRID % token<sym> LID % token<strvalue> STRING % token<address> ADDRESS

																												% token IF % token AND % token OR % token NOT % token EQL % token NOTEQL % token LEQL % token MEQL % token LESS % token MORE % token LIAN % token BING

																												% token LOG % token SIN % token ASIN % token COS % token ACOS % token TAN % token ATAN % token STRIN % token EXP % token POW % token SQR % token MAX % token MIN % token MAXS % token MEAN
...
| table '\n' | table ';' '\n' | strexpr '\n'
{
	if (parser->worktype == 0)
	{
		parser->m_strresult = (char *)$1;
	}
	else
		parser->m_strresult = NULL;
}
| strexpr ';' '\n'
{
	if (parser->worktype == 0)
...
{
	if (parser->worktype == 0)
		$$ = parser->assignchar($1, $3);
}
| strexpr '+' strexpr
{
	if (parser->worktype == 0)
		$$ = parser->AddString($1, $3);
}
| STRID '+' strexpr
{
	if (parser->worktype == 0)
...
int main(int argc, char *argv[])
{
	...
	if (argc == 2)
	{
		...
		while(1){
			thread reason(reasonOnce,re,parser,oa,pl);
			updateData(oa);
			{
				unique_lock <mutex> lck(mtx);
